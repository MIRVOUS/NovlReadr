...
class NovelHall(
    ...
    override suspend fun getChapterText(doc: Document): String =
        withContext(Dispatchers.Default) {
            val content = doc.selectFirst("div#htmlContent")
                ?: return@withContext ""
        ...
        withContext(Dispatchers.Default) {
            tryConnect {
                val img = networkClient
                    .get(bookUrl)
                    .toDocument()
                    .selectFirst(".book-img img, .book-info img, .bookcover img, .cover img")
    ...
    override suspend fun getCatalogSearch(
        ...
        withContext(Dispatchers.Default) {
            tryConnect {
                ...
                val results = doc.selectFirst(".section3.inner.mt30 > table")
    ...
    private fun isLastPage(doc: Document): Boolean =
        when (val nav = doc.selectFirst("div.page-nav")) {
            null -> true
